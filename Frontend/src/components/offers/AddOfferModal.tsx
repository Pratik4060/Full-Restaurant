...
export function AddOfferModal({
  ...
  useEffect(() => {
    ...
    if (initialOffer) {
      ...
      const amountMatch =
        discountText.match(/₹\s*(\d+)/) ?? discountText.match(/(\d+)\s*OFF/);

      setForm({
        title: initialOffer.title,
        description: initialOffer.description,
        discountType: amountMatch && !percentMatch ? "AMOUNT" : "PERCENT",
        discountValue: percentMatch?.[1] ?? amountMatch?.[1] ?? "20",
        validFrom: initialOffer.validFrom
          ? initialOffer.validFrom.slice(0, 10)
          : "",
        validUntil: initialOffer.validUntil
          ? initialOffer.validUntil.slice(0, 10)
          : "",
        minimumOrderAmount: "10",
        isActive: initialOffer.isActive,
      });
      setImagePreviewUrl(initialOffer.imageUrl ?? "");
    } else {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (initialOffer) {
        await dispatch(
          updateOfferThunk({ id: initialOffer.id, data: payload }),
        ).unwrap();
      } else {
      ...
      <form onSubmit={submit} className="w-full max-w-[calc(100vw-56px)]">
        <div className="space-y-4">
          ...
          <div>
            ...
            {submitError === "Image is required" ? (
              <p className="mb-1 text-[11px] text-[#d65c5c]">{submitError}</p>
            ) : null}
                ...
                onChange={(e) =>
                  setForm({
                    ...
                    discountType: e.target
                      .value as OfferFormState["discountType"],
                  })
                }
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    discountValue: clampNonNegativeNumberInput(e.target.value),
                  })
                }
                className="h-10 rounded-[6px] border-[#ded9d1] bg-[#fafafa] px-3 text-[12px]"
                required
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  minimumOrderAmount: clampNonNegativeNumberInput(
                    e.target.value,
                  ),
                })
              }
